...
(function () {
  ...
  console.log('[QR Export] v10.1.0 module loading...');

  class ExportQR {
    constructor() {
      this.initialized = false;
      console.log('[QR Export] v10.1.0 ExportQR instance created');
    }
    ...
    // --- UTILS ---
    detectItemType(item) {
      // [v10.1.1] Ưu tiên item.type (DataManager luôn gán), sau đó item.itemType
      const rawType = (item.type || item.itemType || '').toLowerCase();
      let type = '';
      ...
      if (rawType === 'mold') {
        ...
        console.warn('[QR Export] detectItemType FAILED. item:', JSON.stringify({type: item.type, itemType: item.itemType, MoldID: item.MoldID, CutterID: item.CutterID, TrayID: item.TrayID}));
        return null;
      }

      if (type === 'mold') {
        const id = String(item.MoldID || '').trim();
        const code = String(item.MoldCode || '').trim() || id;
        const name = String(item.MoldName || '').trim();
        if (!id) {
          console.warn('[QR Export] Mold has no MoldID! item:', item);
          return null;
        }
        return { type: 'mold', id, code, name, typeLabel: '金型 / Khuôn' };
      } else if (type === 'tray') {
        const id = String(item.TrayID || '').trim();
        const code = String(item.MoldTrayName || '').trim() || ('Tray ' + id);
        const name = String(item.MoldTrayName || '').trim();
        if (!id) {
          console.warn('[QR Export] Tray has no TrayID! item:', item);
          return null;
        }
        return { type: 'tray', id, code, name, typeLabel: 'トレイ / Khay' };
      } else {
        const id = String(item.CutterID || '').trim();
        const code = String(item.CutterNo || item.CutterCode || '').trim() || id;
        const name = String(item.CutterName || item.Name || '').trim();
        if (!id) {
          console.warn('[QR Export] Cutter has no CutterID! item:', item);
          return null;
        }
        return { type: 'cutter', id, code, name, typeLabel: '切型 / Dao cắt' };
      }
    }
    ...
    generate(item) {
      ...
      console.log('[QR Export] generate() called with item:', item);
      const typeInfo = this.detectItemType(item);
      console.log('[QR Export] typeInfo result:', typeInfo);
      if (!typeInfo) {
    ...
    generateMass(items) {
      this.init();
      console.log('[QR Export] generateMass() called with', items?.length, 'items');
      ...
      const validItems = items.map(item => {
        const result = this.detectItemType(item);
        if (!result) console.warn('[QR Export] detectItemType REJECTED item:', item);
        return result;
      }).filter(Boolean);
      
      console.log('[QR Export] validItems:', validItems.length, 'out of', items.length);
