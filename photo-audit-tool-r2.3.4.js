...
const PhotoAuditTool = {
  ...
  buildCapturePreviewScreen() {
    ...
    const fileNameRow = ce('div', {class: 'pa-form-row'});
    fileNameRow.innerHTML = `
      <label class="pa-label">
        <span class="ja">ファイル名</span> <span class="vi">Tên file</span>
      </label>
  ...
  openCapturePreview(uid) {
    ...
    const timeText = PhotoAuditUtils.formatDateTime(new Date(photo.capturedAt));
    
    this.els.capturePreviewInfo.innerHTML = `
      <div><strong>Nguồn:</strong> ${PhotoAuditUtils.escapeHtml(photo.source)}</div>
      <div><strong>Kích thước:</strong> ${PhotoAuditUtils.escapeHtml(sizeText)}</div>
      <div><strong>Thời gian:</strong> ${PhotoAuditUtils.escapeHtml(timeText)}</div>
    `;
