...
(function() {
    ...
    const UIRenderer = {
        ...
        init() {
            ...
            // ✅ r7.4.2: Sync class inventory-auto-detail theo toggle từ InventoryToolFloat
            document.addEventListener('inventorydetailModalToggle', (e) => {
            const enabled = !!e.detail?.enabled;
            document.body.classList.toggle('inventory-auto-detail', enabled);

            // Nếu tắt detail => đảm bảo selection mode bật để hiện checkbox/cho phép chọn
            if (!enabled) {
                try {
            ...
            // Re-render để hiện checkbox đúng trạng thái
            if (window.UIRenderer && Array.isArray(window.UIRenderer.state?.allResults)) {
                window.UIRenderer.renderQuickCards(window.UIRenderer.state.allResults, false);
            }
            });
            ...
            if (selectionModeToggle) {
                ...
                // ========================================================================
                // R7.4.0: Sync Selection mode theo trạng thái Inventory session
                // - Khi sessionActive=true & multiSelectEnabled=true => SelectionManager.setMode(true)
                // - Khi thoát session hoặc tắt multiSelect => setMode(false)
                // - Không phá logic cũ selection:modeChanged (vẫn giữ để tương thích)
                // ========================================================================

                function syncSelectionModeFromInventory(e) {
                const d = (e && e.detail) ? e.detail : {};
                const inventoryOn = !!d.inventoryOn;
                const sessionActive = !!d.sessionActive;
                const multiSelectEnabled = (d.multiSelectEnabled !== undefined) ? !!d.multiSelectEnabled : true;

                const shouldEnable = !!(inventoryOn && sessionActive && multiSelectEnabled);

                // Ưu tiên SelectionManager (single source of truth)
                if (window.SelectionManager && typeof window.SelectionManager.setMode === 'function') {
                    window.SelectionManager.setMode(shouldEnable);
                    return; // SelectionManager sẽ tự dispatch selection:modeChanged + selectionmodeChanged
                }

                // Fallback: update SelectionState + emit selection:modeChanged (giữ tương thích)
                if (!window.SelectionState) window.SelectionState = { active: false, items: [] };
                window.SelectionState.active = shouldEnable;

                document.dispatchEvent(new CustomEvent('selection:modeChanged', {
                    detail: { enabled: shouldEnable, source: 'ui-renderer-inventory-sync' }
                }));
                }

                // Listen both new & legacy events
                document.addEventListener('inventorymodeChanged', syncSelectionModeFromInventory);
                document.addEventListener('inventory:modeChanged', syncSelectionModeFromInventory);
        ...
        renderQuickCards(items, append = false) {
            ...
            itemsToRender.forEach((item, localIdx) => {
                ...
                if (isAuditedToday) el.classList.add("audited-today")
                ...
                // ✅ Kiểm tra ngày hiển thị có phải hôm nay không (dùng lại biến today đã có)
                let isDateToday = false;
                if (displayDate) {
                    try {
                        const dateStr = new Date(displayDate).toISOString().split('T')[0];
                        isDateToday = (dateStr === today);
                    } catch (e) {
                        isDateToday = false;
                    }
                }
                const dateClass = isDateToday ? 'card-date date-today' : 'card-date';
        ...
        setupCardEvents(wrap) {
            ...
            // ✅ Lấy card 1 LẦN DUY NHẤT
            const card = e.target.closest('.result-card[data-id][data-type]');
            if (!card) return;

            const inventoryActive = !!window.InventoryState?.active;
            const detailModalEnabled = window.InventoryToolFloat?.getDetailModalEnabled?.() ?? true;

            // ✅ r7.4.2: Khi Inventory ON và Detail Modal OFF => click card để CHỌN
            if (inventoryActive && !detailModalEnabled) {
            const itemId = card.getAttribute('data-id');
            const itemType = (card.getAttribute('data-type') || '').toLowerCase();
            const index = parseInt(card.getAttribute('data-index'), 10);
            const itemData = (!isNaN(index) && this.state?.allResults?.[index]) ? this.state.allResults[index] : (card.itemData || null);
            ...
            try {
                ...
                window.SelectionState = window.SelectionState || { active: false, items: [] };
                window.SelectionState.active = true;
                document.dispatchEvent(new CustomEvent('selectionmodeChanged', { detail: { enabled: true, source: 'ui-renderer-r7.4.2' } }));
                }
            } catch (_) {}

            // 3) Kiểm tra multi-select từ ToolFloat
            let allowMulti = false;  // ❌ SỬA: mặc định false thay vì true
            try {
                if (window.InventoryToolFloat && typeof window.InventoryToolFloat.getMultiSelectEnabled === 'function') {
                allowMulti = !!window.InventoryToolFloat.getMultiSelectEnabled();
                } else {
                // ❌ SỬA: thêm fallback đọc từ ToolState
                allowMulti = !!(window.InventoryToolFloat && window.InventoryToolFloat.ToolState && window.InventoryToolFloat.ToolState.multiSelectEnabled);
                }
            } catch (_) {}

            // 4) Nếu không cho chọn nhiều => clear trước khi chọn item mới
            try {
                const isAlreadySelected = (window.SelectionManager && typeof window.SelectionManager.isSelected === 'function')
                ? !!window.SelectionManager.isSelected(itemId, itemType)
                : false;

                if (!allowMulti && !isAlreadySelected && window.SelectionManager && typeof window.SelectionManager.clear === 'function') {
                window.SelectionManager.clear();
                }
            } catch (_) {}
            ...
            console.log('[UIRenderer] r7.4.2: Inventory ON + Detail OFF => toggled selection', itemType, itemId, 'allowMulti:', allowMulti);
            return;
            }
        ...
        updateCheckInBadge(item) {
            ...
            try {
                ...
                if (status === 'check-in' || status.includes('in')) {
                    ...
                    const logDate = latestLog.Timestamp ? latestLog.Timestamp.split("T")[0] : null
                    const today = new Date().toISOString().split("T")[0]
                    if ((status === "audit" || status.toUpperCase() === "AUDIT" || status.includes("audit")) && logDate === today) {
    ...
    // ---- FIX: Mobile bottom nav Inventory button luôn mở được ----
    (function bindNavInventoryOnce() {
    if (window.__navInventoryBtnBound) return;
    window.__navInventoryBtnBound = true;

    // capture=true để chạy trước các handler khác (LowerTabs / MobileController)
    document.addEventListener('click', function (e) {
        const btn = e.target && e.target.closest ? e.target.closest('#nav-inventory-btn') : null;
        if (!btn) return;

        e.preventDefault();
        e.stopPropagation();

        // Ưu tiên gọi InventoryUI (nếu có)
        if (window.InventoryUI && typeof window.InventoryUI.openSettings === 'function') {
        window.InventoryUI.openSettings();
        return;
        }

        // Fallback: gọi InventoryManager (nhiều bản có openSettings -> dispatch inventorytoggle)
        if (window.InventoryManager && typeof window.InventoryManager.openSettings === 'function') {
        window.InventoryManager.openSettings();
        return;
        }

        // Fallback 2: bấm nút desktop (nút này có trong HTML)
        const desktopBtn = document.getElementById('btn-inventory-settings');
        if (desktopBtn && typeof desktopBtn.click === 'function') {
        desktopBtn.click();
        return;
        }

        // Fallback 3: phát event trực tiếp
        try {
        document.dispatchEvent(new CustomEvent('inventorytoggle', { detail: { open: true, source: 'nav-inventory-btn' } }));
        } catch (err) {}
    }, true);
    })();
