...
(function () {
  ...
  function buildStatusRows(item, itemType, data) {
    var out = [];
    var logs = toArray(data && data.statuslogs).concat(toArray(data && data.webstatuslogs));
  ...
  function buildLocationRows(item, itemType, data) {
    var out = [];
    var logs = toArray(data && data.locationlog).concat(toArray(data && data.weblocationlog));
    for (var i = 0; i < logs.length; i++) {
      var row = logs[i];
  ...
  function resolveCompanyPair(data, row) {
    var fromVal = normId(row.FromCompanyID) || normId(row.FromCompany);
    var toVal = normId(row.ToCompanyID) || normId(row.ToCompany);

    var fromName = findCompanyName(data, fromVal);
    var toName = findCompanyName(data, toVal);
  ...
  function buildShipRows(item, itemType, data) {
    var out = [];
    var logs = toArray(data && data.shiplog).concat(toArray(data && data.webshiplog));
    for (var i = 0; i < logs.length; i++) {
      var row = logs[i];
  ...
  function normalizeTeflonStatus(raw) {
    ...
    var u = s.toUpperCase();
    var txt = s;
    if (u === 'REQUESTED' || u === 'REQUEST') txt = '依頼 / Requested';
    else if (u === 'SENT' || u === 'SHIP') txt = '送付 / Sent';
    else if (u === 'RECEIVED' || u.indexOf('受') >= 0 || u.indexOf('完') >= 0) txt = '受領 / Received';
    else if (u === 'DELETED') txt = '削除 / Deleted';

    if (u.indexOf('RECEIVED') >= 0 || s.indexOf('受') >= 0 || s.indexOf('完') >= 0) return { text: txt, kind: 'in' };
    if (u.indexOf('SENT') >= 0 || u.indexOf('SHIP') >= 0 || s.indexOf('送') >= 0) return { text: txt, kind: 'out' };
    if (u.indexOf('REQUEST') >= 0 || u.indexOf('PLAN') >= 0 || s.indexOf('依頼') >= 0) return { text: txt, kind: 'audit' };
    return { text: txt, kind: 'unknown' };
  }
  ...
  function buildCommentRows(item, itemType, data) {
    var out = [];
    var logs = toArray(data && data.usercomments).concat(toArray(data && data.webusercomments));
    for (var i = 0; i < logs.length; i++) {
      var row = logs[i];
      var ok = isRowForItemByIds(row, item, itemType, 'MoldID', 'CutterID');
  ...
  function makeTimelineEvents(datasets) {
    ...
    for (var j = 0; j < datasets.location.length; j++) {
      ...
      out.push({
        ...
        title: 'ロケーション',
        summaryHtml: escapeHtml(l.OldRackLayer || '-') + ' <i class="fas fa-arrow-right" style="color:#94a3b8;font-size:0.85em;margin:0 4px;"></i> ' + escapeHtml(l.NewRackLayer || '-'),
        meta1: l.Employee || '',
        meta2: l.notes || ''
      });
    }
    ...
    for (var k = 0; k < datasets.ship.length; k++) {
      ...
      out.push({
        ...
        icon: 'fa-truck',
        title: '移動 / Transfer',
        summaryHtml: escapeHtml(sh.FromCompany || '-') + ' <i class="fas fa-arrow-right" style="color:#94a3b8;font-size:0.85em;margin:0 4px;"></i> ' + escapeHtml(sh.ToCompany || '-'),
        meta1Html: shipBadge,
        meta2: sh.ShipNotes || ''
      });
    }
    ...
    for (var t = 0; t < datasets.teflon.length; t++) {
      ...
      var metaDetails = [];
      if (tf.RequestedDate) metaDetails.push('依頼: ' + formatDate(tf.RequestedDate));
      if (tf.SentDate) metaDetails.push('送付: ' + formatDate(tf.SentDate));
      if (tf.ReceivedDate) metaDetails.push('受領: ' + formatDate(tf.ReceivedDate));
      if (tf.Supplier) metaDetails.push('業者: ' + tf.Supplier);
  ...
  function renderTimelineHtml(events, query, sortDir) {
    ...
    parts.push(
      '<style>' +
      '.dhs-modern-timeline { position:relative; padding-left:4px; padding-top:4px; }' +
      '.dhs-modern-timeline::after { content:""; position:absolute; width:2px; background:#e2e8f0; top:8px; bottom:16px; left:14px; z-index:1; }' +
      '.dhs-modern-timeline .timeline-item:last-child { padding-bottom: 0; }' +
      '.dhs-modern-timeline .timeline-content { transition: border-color 0.15s; }' +
      '.dhs-modern-timeline .timeline-content:hover { border-color: #cbd5e1; }' +
      '</style>'
    );

    parts.push('<div class="history-timeline dp-dhs-timeline dhs-modern-timeline">');
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        var icColor = colorMap[e.group] || '#64748b';
  
        parts.push(
          '<div class="timeline-item" data-type="' + escapeHtml(e.group) + '" style="position:relative; display:flex; padding-bottom:14px; margin-bottom:0;">' +
          '<div class="timeline-icon" style="flex-shrink:0; width:22px; height:22px; border-radius:50%; background:#fff; border:2px solid ' + icColor + '80; display:flex; align-items:center; justify-content:center; font-size:10px; margin-right:12px; margin-top:2px; z-index:3; color:' + icColor + ';">' +
            '<i class="fas ' + escapeHtml(e.icon) + '"></i>' +
          '</div>' +
          '<div class="timeline-content" style="flex-grow:1; background:#fff; border:1px solid #e2e8f0; border-radius:6px; padding:6px 12px; box-shadow:0 1px 2px rgba(0,0,0,0.02);">' +
            '<div class="timeline-header" style="margin-bottom:2px; display:flex; justify-content:space-between; align-items:center;">' +
              '<span class="timeline-type" style="font-size:11px;font-weight:700;letter-spacing:0.02em; color:#475569;">' + escapeHtml(e.title) + '</span>' +
              '<span class="timeline-date" style="font-size:11px; color:#94a3b8;">' + escapeHtml(e.dateText) + '</span>' +
            '</div>' +
            '<div class="timeline-body" style="display:flex; flex-direction:column; gap:2px;">' +
              '<div class="dp-dhs-tl-summary" style="font-size:13px; font-weight:600; color:#1e293b;">' + summaryHtml + '</div>' +
              (metaHtml ? ('<div class="dp-dhs-tl-meta-wrap" style="color:#64748b; font-size:11px; font-weight:500;">' + metaHtml + '</div>') : '') +
            '</div>' +
          '</div>' +
          '</div>'
        );
  ...
  function buildToolbarHtml(state) {
    var g = state.group || 'ALL';

    function chip(label, key, icon, clr) {
      var cls = 'dp-dhs-chip' + (g === key ? ' active' : '');
      var cStyle = (g === key && clr) ? (' style="color:' + clr + ';border-color:' + clr + ';background:' + clr + '1a"') : '';
      return '<button type="button" class="' + cls + '" data-dhs-group="' + escapeHtml(key) + '"' + cStyle + '>' +
        '<i class="fas ' + escapeHtml(icon) + '"></i>' +
    ...
    return (
      ...
      chip('すべて / Tất cả', 'ALL', 'fa-stream') +
      chip('ステータス / Status', 'STATUS', 'fa-clipboard-list', '#0ea5e9') +
      chip('棚移動 / Chuyển Kệ', 'LOCATION', 'fa-map-marker-alt', '#f59e0b') +
      chip('移動 / Transfer', 'SHIP', 'fa-truck', '#8b5cf6') +
      chip('テフロン / Teflon', 'TEFLON', 'fa-spray-can', '#ec4899') +
      chip('コメント / Comments', 'COMMENT', 'fa-comment-dots', '#10b981') +
      '<span class="dp-dhs-sep" aria-hidden="true"></span>' +
      '<select class="dp-dhs-select" title="Sắp xếp">' +
      '<option value="DESC"' + (state.sortDir === 'DESC' ? ' selected' : '') + '>最新 / Mới nhất</option>' +
      '<option value="ASC"' + (state.sortDir === 'ASC' ? ' selected' : '') + '>最古 / Cũ nhất</option>' +
      '</select>' +
      '<input class="dp-dhs-search" type="text" value="' + escapeHtml(state.query || '') + '" placeholder="検索 / Tìm kiếm..." />' +
      '<button class="dp-dhs-btn-quick" data-action="teflon" title="テフロン依頼 / Tạo Lệnh Teflon" style="margin-left:auto; background:#2563eb; color:white; border:none; padding:4px 12px; border-radius:4px; font-weight:600; cursor:pointer;"><i class="fas fa-plus"></i> テフロン依頼 / Mạ Teflon</button>' +
      '</div>'
    );
  }
  ...
  function renderGroupBody(state) {
    ...
    var fed = state.timeline;
    if (state.group !== 'ALL') {
       fed = fed.filter(function(x) { return x.group === state.group; });
    }
    return '<div class="modal-section">' + renderTimelineHtml(fed, q, dir) + '</div>';
  }
  ...
  function applyStateRender(state) {
    ...
    if (countEl) {
      var fed = state.timeline;
      if (state.group !== 'ALL') fed = fed.filter(function(x) { return x.group === state.group; });
      if (state.query) fed = fed.filter(function (r) { return textMatchRow(r, state.query); });
      countEl.textContent = String(fed.length);
    }
  }
  ...
  function bindUI(state) {
    ...
    var tbn = host.querySelector('.dp-dhs-btn-quick');
    if (tbn && !tbn.dataset.dpDhsBound) {
      tbn.dataset.dpDhsBound = '1';
      tbn.addEventListener('click', function() {
         if (window.TeflonProcessing && typeof window.TeflonProcessing.openModal === 'function') {
             window.TeflonProcessing.openModal(state.item);
         } else {
             if(window.notify) window.notify.error('Module TeflonProcessing chưa sẵn sàng.');
         }
      });
    }
  }
  ...
  function render(hostEl, opts) {
    ...
    if (!item) {
      host.innerHTML = '<div class="modal-section"><div class="section-header"><i class="fas fa-history"></i><span>デバイス状態履歴 / Lịch sử trạng thái thiết bị</span></div><p class="no-data">デバイスがありません / Không có thiết bị.</p></div>';
      return;
    }
    ...
    host.innerHTML = (
      ...
      '<span>デバイス状態履歴 / Lịch sử trạng thái thiết bị</span>' +
      '<span class="dp-dhs-count">0</span>' +
      '</div>' +
      '<div class="info-message">デバイス / Thiết bị: <b>' + escapeHtml(code || '-') + '</b>. タイムライン表示 / Hiển thị dạng Timeline.</div>' +
