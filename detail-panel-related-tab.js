...
(function (global) {
  ...
  function openJobDetailPopup(jobId) {
    ...
    function fmtDate(v) {
      ...
      if (!v || String(v).trim() === '-' || String(v).trim() === '') return 'N/A';
  ...
  function renderLifecycleTreeBody(item, itemType) {
    ...
    if (relatedJobs.length === 0) {
      ...
      for (var k = 0; k < relatedJobs.length; k++) {
        ...
        var rawJDate = jb.DateEntry || jb.UpdatedAt || '';

        var jDate = fmtDateJPGlobal(rawJDate);
        ...
        if (dlItems.length > 0) {
          ...
          for (var idx = 0; idx < dlItems.length; idx++) {
            ...
            var rawDlDate = dl.ProcessingDeadlineDate || dl.ProcessingDeadline || '';

            var dlDate = fmtDateJPGlobal(rawDlDate);
