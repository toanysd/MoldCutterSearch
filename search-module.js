...
class SearchModule {
  ...
  attachEventListeners() {
    ...
    // NEW Feature: 2-finger Swipe down anywhere to clear search & open Virtual Keyboard
    let tsY = 0;
    let tsX = 0;
    let isTwoFingerSwipe = false;
    document.addEventListener('touchstart', (e) => {
      // Bắt đầu 2 ngón tay chạm cùng lúc
      if (e.touches.length === 2) {
        isTwoFingerSwipe = true;
        tsY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
        tsX = (e.touches[0].clientX + e.touches[1].clientX) / 2;
      } else {
    ...
    document.addEventListener('touchend', (e) => {
      ...
      const deltaY = teY - tsY;
      const deltaX = Math.abs(teX - tsX);
      isTwoFingerSwipe = false; // reset

      // Swipe down gesture (Dọc nhiều hơn ngang & deltaY > 80px)
      if (deltaY > 80 && deltaY > deltaX) {
        console.log('[SearchModule] 2-finger swipe down detected. Clearing search and opening Virtual Keyboard.');
        this.clearSearch();
        if (window.VirtualKeyboard && typeof window.VirtualKeyboard.open === 'function') {
  ...
  emitSearchEvent(query) {
    if (window.CurrentSearchContext === 'plastic') {
      const pEvent = new CustomEvent('plasticSearch', { detail: { query: query, timestamp: Date.now() } });
      document.dispatchEvent(pEvent);
      console.log('[SearchModule] Emitted plasticSearch:', query);
      return;
    }
