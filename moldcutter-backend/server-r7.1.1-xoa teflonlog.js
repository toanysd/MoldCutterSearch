...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'OK',
    message: 'Server V7.7.7-r7.1.0 running (Teflon Delete Support)', // ✅ ĐỔI message
...
app.post("/api/deletelog", async (req, res) => {
  ...
  try {
    ...
    // ========================================
    // ✅ NEW r7.1.0: DELETE LOG (UNIFIED)
    // POST /api/delete-log - Xóa log từ bất kỳ file nào
    // Body: { filename, logId }
    // Supports: teflonlog.csv, statuslogs.csv, locationlog.csv
    // ========================================
    app.post('/api/delete-log', async (req, res) => {
      ...
      try {
        ...
        } else if (filename === 'shiplog.csv') {  // <-- THÊM DÒNG NÀY
           idField = 'ShipID';  
        } else {
          return res.status(400).json({ 
            success: false, 
