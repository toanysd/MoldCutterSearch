...
(function (global) {
  ...
  function ensureSkeleton() {
    ...
    tabCompany.addEventListener('click', function () {
      RackRelocation.currentTab = 'company';
    ...
    document.getElementById('locmove-close-btn').addEventListener('click', function () { RackRelocation.close(); });
    document.getElementById('locmove-cancel-btn').addEventListener('click', function () { RackRelocation.close(); });
  ...
  function submitMove() {
    var item = RackRelocation.currentItem;
    if (!item) return;
    ...
    submitBtn.disabled = true;
      ...
      fetch(apiUrl, {
        ...
        .catch(function (err) {
          console.warn('RackRelocation API error, continuing locally', err);
        })
        .finally(function () {
          // 2. Update local data
          var allData = getAllData();
  ...
  RackRelocation.apiMoveRackLayer = function(item, layerId, empId, note) {
    ...
    return fetch(apiUrl, {
      ...
      .catch(function (err) {
        console.warn('RackRelocation.apiMoveRackLayer API error', err);
  ...
  global.RackRelocation = RackRelocation;
