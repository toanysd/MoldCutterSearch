...
(function (global) {
  ...
  async function persistFileSizesToDb(store, rows) {
    ...
    try {
      var res = await client.from(store.tableName).upsert(payload, { onConflict: 'id' });
      // Nếu lỗi schema (chưa có cột file_size) hoặc RLS chặn → bỏ qua im lặng
      if (res && res.error) {
        // Không throw, chỉ log nhẹ để debug
        log('persistFileSizes skipped (DB may lack file_size column or RLS):', res.error.message || res.error.code);
      }
    } catch (e) {
  ...
  async function injectSignedUrls(client, bucketId, rows, ttl) {
    ...
    rows.forEach(function (r, i) {
      allPaths.push(thumbPaths[i] || mainPaths[i] || null);
    });
    var validPaths = allPaths.filter(Boolean);
    ...
    try {
      ...
      rows.forEach(function (r, i) {
        var usedPath = allPaths[i];
        if (usedPath && signedMap[usedPath]) {
          r.public_url = signedMap[usedPath];
          r.publicurl = signedMap[usedPath];
          r.publicUrl = signedMap[usedPath];
          r.thumb_public_url = signedMap[usedPath];
          r.thumbpublicurl = signedMap[usedPath];
          r.thumbPublicUrl = signedMap[usedPath];
        }
      });
    } catch (e) { }
