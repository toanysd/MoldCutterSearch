...
class MobileDetailModal {
    ...
    */
    ...
    triggerTeflon(item, type) {
      ...
      /**
         * ✅ R7.1.6 FIX: Trigger Photo Audit with auto-fill mold data
         */
        triggerPhotoAudit(item, type) {
            ...
            alert('Không có dữ liệu để chụp ảnh');
            return;
            }
            ...
            // Wait for PhotoAuditTool to be ready
            setTimeout(() => {
            ...
            // ✅ FIX: Prepare mold data and pass to PhotoAuditTool
            const moldData = {
                moldId: item.MoldID || item.CutterID || null,
                moldCode: item.MoldCode || item.CutterNo || '',
                moldName: item.MoldName || item.CutterName || item.MoldCode || item.CutterNo || ''
            };

            console.log('[MobileModal] 📸 Opening Photo Audit with data:', moldData);

            try {
                // ✅ Pass mold data to PhotoAuditTool
                window.PhotoAuditTool.openSettings({ mold: moldData });
                console.log('[MobileModal] ✅ Photo Audit opened successfully');
            } catch (error) {
