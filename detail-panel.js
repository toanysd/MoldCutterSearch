...
(function () {
  ...
  class DetailPanel {
    ...
    loadDataReferences() {
      ...
      try {
        ...
        if (typeof window.DataManager !== 'undefined' && window.DataManager.data) {
          ...
          this.data.locations = d.locations || [];
          ...
          this.data.companies = d.companies || []; this.data.usercomments = d.usercomments || [];
          ...
          this.data.webstatuslogs = d.webstatuslogs || [];

          this.data.weblocationlog = d.weblocationlog || [];

          this.data.webshiplog = d.webshiplog || [];

          this.data.webusercomments = d.webusercomments || [];
    ...
    handleQuickAction(action) {
      ...
      if (String(action).toLowerCase().trim() === 'scrap') {

        try {

          if (window.ScrapWizardModule && typeof window.ScrapWizardModule.openModal === 'function') {

            window.ScrapWizardModule.openModal(this.currentItem);

            return;

          } else {

            this.notify('Module Hủy khuôn chưa sẵn sàng.', 'error');
      ...
      // Action Print / Excel Export (Detail Print Module)

      if (String(action).toLowerCase().trim() === 'print') {

        try {

          if (window.DetailPrintModule && typeof window.DetailPrintModule.openModal === 'function') {

            window.DetailPrintModule.openModal(this.currentItem, this.currentItemType);
    ...
    renderDesktopHero(item, type) {
      ...
      return (
        ...
        '<span class="dp-hero-v location-link qt-interactive" data-rack-id="' + (item?.rackNo || item?.RackID || '') + '" data-layer-id="' + (item?.RackLayerID || '') + '" style="cursor:pointer; display:inline-flex; align-items:center; color:#0369a1;">' +

        '<img class="card-thumb-img" data-devicetype="' + ((item?.RackLayerID || '') ? 'racklayer' : ((item?.rackNo || item?.RackID || '') ? 'rack' : '')) + '" data-deviceid="' + (item?.RackLayerID || item?.rackNo || item?.RackID || '') + '" src="" style="display:none; width: 16px; height: 16px; border-radius: 3px; margin-right: 4px; object-fit: cover;">' +

        '<i class="placeholder-icon fas ' + ((String(locationText || '').toLowerCase().includes('社外保管') || String(locationText || '').toLowerCase().includes('lưu trữ')) ? 'fa-building' : 'fa-map-marker-alt') + '" style="margin-right:4px; color:' + ((String(locationText || '').toLowerCase().includes('社外保管') || String(locationText || '').toLowerCase().includes('lưu trữ')) ? '#ea580c' : '#0284c7') + ';"></i>' +
    ...
    renderPreviewHero(item, type) {
      ...
      const disposing = isMold ? (item?.MoldDisposing || item?.Disposing || (item?.MoldSt === 'DISPOSED' ? 'Đã hủy' : '')) : (item?.Disposing || (item?.CutterSt === 'DISPOSED' ? 'Đã hủy' : ''));
        ...
        <div class="dp-actions-grid">
          ...
          <button class="dp-action-btn dp-action-btn--inout" data-action="inout" type="button" title="入出庫・位置変更 / Nhập xuất, chuyển vị trí">
            ...
            <div class="dp-action-btn-texts">
              ...
              <span class="dp-action-label-ja">入出庫・位置</span>

              <span class="sub dp-action-label-vi">Nhập xuất, Vị trí</span>
          ...
          <button class="dp-action-btn dp-action-btn--move" data-action="move" type="button" title="移動・返却 / Di chuyển, Trả khuôn">
          ...
          <button class="dp-action-btn dp-action-btn--weight" data-action="qu-weight" type="button" title="重量更新 / Khối lượng">
          ...
          <button class="dp-action-btn dp-action-btn--scrap" data-action="scrap" type="button" title="廃棄 / Hủy khuôn">
...
`;
    ...
    async hydrateDesktopPhotoPreviewFromSupabase() {
      ...
      try {
        ...
        // (C) Hydrate thumb location

        try {

          const locImg = this.panel.querySelector('.location-link img.card-thumb-img');

          if (locImg) {

            const dtype = String(locImg.dataset.devicetype || '').toLowerCase();

            const did = String(locImg.dataset.deviceid || '').trim();

            if (dtype && did && window.DevicePhotoStore) {

              const thumbUrl = await window.DevicePhotoStore.getThumbnailUrl(dtype, did);

              if (thumbUrl) {

                locImg.onload = function () { locImg.style.display = 'inline-block'; };

                locImg.src = thumbUrl;

                const ph = locImg.parentElement.querySelector('.placeholder-icon');
        ...
        <div class="modal-section">
          ...
          <div style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,0.10);background:#fff;height:220px;min-height:220px">
            ...
            <img data-dp-thumb-img="1"
                ...
                onclick="if(window.openGlobalPhotoZoom) window.openGlobalPhotoZoom(this.dataset.dpFullUrl || this.src); event.stopPropagation();" />
            ...
            <div style="position:absolute;top:10px;right:10px;display:flex;gap:8px;z-index:3">
              ...
              <button type="button" data-dp-photo-download="1" title="画像ダウンロード / Tải ảnh"
    ...
    renderOverviewSection(item, type) {
      ...
      const rawFirstShipment = safeText(
      ...
      const firstShipment = formatToJPDate(rawFirstShipment);
      ...
      const nameBadge = `
        ...
        <div style="font-size: 16px; font-weight: 800; color:#0f172a; line-height: 1.4; display: flex; align-items: center; gap: 8px;">
           ...
           <span style="font-size: 12px; color: #94a3b8; font-weight: 600; font-family: monospace;">#${isMold ? (item.MoldID || '-') : (item.CutterID || item.ID || '-')}</span>
    ...
    renderMoldBasicInfoFull(item) {
      ...
      if (job) {
        ...
        if (dd) {
          ...
          madeDate = !isNaN(d.getTime()) ? `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日` : this.safeText(dd);
    ...
    renderCutterBasicInfoFull(item) {
      ...
      const rawCutterEntry = item?.CutterManufactureDate ?? item?.CutterEntry;

      let cutterEntry = this.safeText(rawCutterEntry);

      if (rawCutterEntry && String(rawCutterEntry).trim() !== '-') {

        const d = new Date(rawCutterEntry);

        if (!isNaN(d.getTime())) {

          cutterEntry = `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日`;
    ...
    renderProductInfoFull(item, type) {
      ...
      const rawProdDate = (type === 'mold') ? (item?.ProductionDate || job?.DeliveryDeadline || item?.displayDate || '-') : (item?.CutterManufactureDate || '-');

      const productionDate = formatToJPDate(rawProdDate);
    ...
    renderStatusNotesSection(item, type) {
      ...
      const isDisplayableStatus = (v) => {

        const str = String(v || '').trim();

        const low = str.toLowerCase();

        if (!str || str === '-' || low === 'false' || low === '0' || low === 'no') return false;
      ...
      const returningVal = isMold ? (item?.MoldReturning || item?.Returning) : '';

      const disposingVal = isMold ? (item?.MoldDisposing || item?.Disposing) : '';



      const isReturning = isDisplayableStatus(returningVal);

      const isDisposing = isDisplayableStatus(disposingVal);
      ...
      if (isReturning) {
        ...
        const retDate = isMold && item?.MoldReturnedDate ? ` (${item.MoldReturnedDate})` : '';
        ...
        html += `<div class="info-message" style="background:#fef3c7;border-left:4px solid #f59e0b;padding:10px;margin-bottom:10px;">
          ...
          <strong>返却金型 / Đã trả khuôn:</strong> ${this.escapeHtml(returningVal)}${retDate}
      ...
      if (isDisposing) {
        ...
        const dispDate = isMold && item?.MoldDisposedDate ? ` (${item.MoldDisposedDate})` : '';
        ...
        html += `<div class="info-message" style="background:#fee2e2;border-left:4px solid #dc2626;padding:10px;margin-bottom:10px;">
          ...
          <strong>廃棄金型 / Đã hủy khuôn:</strong> ${this.escapeHtml(disposingVal)}${dispDate}
      ...
      if (!isReturning && !isDisposing && !moldNotes && !latestComment) {
  ...
  `;
    ...
    renderStatusTab() {
      ...
      const disposing = item.MoldDisposing || item.Disposing || ((item.MoldSt === 'DISPOSED' || item.CutterSt === 'DISPOSED') ? 'Đã hủy' : '');
    ...
    getStatusHistory(itemId, kind) {
      ...
      const logs = (this.data.statuslogs || []).concat(this.data.webstatuslogs || []);
    ...
    getShipHistory(itemId, kind) {
      ...
      const logs = (this.data.shiplog || []).concat(this.data.webshiplog || []);
    ...
    getComments(itemId, itemType) {
      ...
      const comments = (this.data.usercomments || []).concat(this.data.webusercomments || []);
  ...
  (function () {
    ...
    function ensureStyle() {
      ...
      st.textContent = `
        ...
        .mcs-qv{width:min(1200px,96vw);height:min(86vh,860px);background:rgba(255,255,255,.98);border-radius:16px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.25);border:1px solid rgba(0,0,0,.12);display:flex;flex-direction:column;pointer-events:auto;}
        ...
        .mcs-qv-body{flex:1 1 auto;position:relative;background:#0b1220;display:flex;align-items:center;justify-content:center;min-height:0;min-width:0;overflow:auto;}

        .mcs-qv-body.zoomed{align-items:flex-start;justify-content:flex-start;}

        .mcs-qv-img{max-width:100%;max-height:100%;object-fit:contain;display:none;cursor:zoom-in;transition:transform 0.2s ease;}

        .mcs-qv-img.zoomed{max-width:none;max-height:none;width:150%;height:auto;cursor:zoom-out;transform-origin:top left;}

        .mcs-qv-spin{position:absolute;color:#e2e8f0;font-weight:800;opacity:.9;}

        .mcs-qv-note{position:absolute;bottom:10px;left:12px;right:12px;color:#cbd5e1;font-size:12px;opacity:.9;pointer-events:none;}
    ...
    function ensureDom() {
      ...
      bd.innerHTML = `
        ...
        <div class="mcs-qv" role="dialog" aria-modal="true">
          ...
          <div class="mcs-qv-body">
            ...
            <div class="mcs-qv-note">Vuốt phải để đóng • Nhấp vào ảnh để Phóng to</div>
      ...
      const imgNode = bd.querySelector('.mcs-qv-img');

      const bodyNode = bd.querySelector('.mcs-qv-body');
      ...
      imgNode.addEventListener('click', function (e) {
        ...
        if (this.classList.contains('zoomed')) {
          ...
          bodyNode.classList.remove('zoomed');

        } else {

          this.classList.add('zoomed');

          bodyNode.classList.add('zoomed');

          setTimeout(() => {

            bodyNode.scrollLeft = (bodyNode.scrollWidth - bodyNode.clientWidth) / 2;

            bodyNode.scrollTop = (bodyNode.scrollHeight - bodyNode.clientHeight) / 2;
    ...
    function close() {
      ...
      if (bodyNode) bodyNode.classList.remove('zoomed');
