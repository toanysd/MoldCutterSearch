/* ========================================================================
 * MOBILE PANEL CONTROLLER R7.1.7
 * ========================================================================
 * Quản lý show/hide chi tiết & action buttons trên iPhone
 * - Show detail panel khi click result card
 * - Bật action buttons
 * - Chèn "View full detail" link
 * - Bottom nav + Floating action bar + Inventory & Photo Audit
 * Created: 2025-11-07
 * Last Updated: 2025-12-23 (r7.1.7 - Full version)
 * ======================================================================== */
...
class MobilePanelController {
  ...
  /* ============================================================================
   * R7.1.6/7.1.7 OPEN PHOTO AUDIT WITH AUTO-FILL DATA
   * ============================================================================ */
  openPhotoAuditWithData() {
    if (!window.PhotoAuditTool || typeof window.PhotoAuditTool.openSettings !== 'function') {
      ...
      alert('Công cụ Photo Audit có lỗi, vui lòng liên hệ admin');
      return;
    }

    // Get currently selected mold data if any for auto-fill
    let moldIdOrCode = null;

    // Try to get from last selected item
    const selectedItemId =
      document.body.dataset.selectedItemId || document.body.dataset.lastSelectedItemId;

    if (selectedItemId && window.DataManager && window.DataManager.data) {
      const allData = window.DataManager.data;

      // Search in molds
      let mold = allData.molds.find(
        (m) => m.MoldID == selectedItemId || m.MoldCode == selectedItemId
      );

      // Search in cutters if not found in molds
      if (!mold && allData.cutters) {
        const cutter = allData.cutters.find(
          (c) => c.CutterID == selectedItemId || c.CutterNo == selectedItemId
        );
        if (cutter) {
          moldIdOrCode = cutter.CutterNo || cutter.CutterID;
        }
      }

      // Use mold code/ID
      if (mold) {
        moldIdOrCode = mold.MoldCode || mold.MoldID;
      }

      console.log('[MobilePanelController] Auto-fill Photo Audit with:', moldIdOrCode);
    }

    // ✅ CORRECT: Open PhotoAuditTool
    try {
      // Always call openSettings() first (no params)
      window.PhotoAuditTool.openSettings();

      // Then auto-fill mold if available
      if (moldIdOrCode && typeof window.PhotoAuditTool.preselectMold === 'function') {
        window.PhotoAuditTool.preselectMold(moldIdOrCode);
        console.log('[MobilePanelController] Photo Audit opened with auto-fill:', moldIdOrCode);
      } else {
        console.log('[MobilePanelController] Photo Audit opened (manual entry mode)');
      }
    } catch (error) {
      console.error('[MobilePanelController] Error opening Photo Audit', error);
  ...
  openMobileDetailModal(itemId, itemType, cardIndex) {
    ...
    requestAnimationFrame(() => {
      ...
      // Tìm item theo type
      if (itemType === 'mold') {
        item = allData.molds.find(
          (m) => m.MoldID === itemId || m.MoldCode === itemId
        );
      } else if (itemType === 'cutter') {
        item = allData.cutters.find(
          (c) => c.CutterID === itemId || c.CutterNo === itemId
        );
      }
  ...
  checkFilterActive() {
    ...
    const activeCategory = Array.from(categoryTabs).find(
      (tab) =>
    ...
    navItems.forEach((item) => {
      item.addEventListener('click', (e) => {
        ...
        switch (tab) {
          ...
          case 'location':
          case 'inventory':
            console.log('[MobilePanelController] Bottom nav clicked: location');
  
            // ✅ Gọi InventoryUI r8.0 thay vì legacy InventoryManager
            if (window.InventoryUI && typeof window.InventoryUI.showSetupModal === 'function') {
              window.InventoryUI.showSetupModal();
              console.log('[MobilePanelController] ✅ InventoryUI r8.0 opened');
            } else if (window.InventoryManager && typeof window.InventoryManager.showSetupModal === 'function') {
              // Fallback: legacy InventoryManager
              window.InventoryManager.showSetupModal();
              console.log('[MobilePanelController] ⚠️ Legacy InventoryManager opened');
            } else {
              console.error('[MobilePanelController] ❌ No Inventory module available');
          ...
          case 'photo-audit':
            ...
            // 1. Đóng floating action bar
            this.hideFloatingActionBar();

            // 2. ✅ FIX: Wait for PhotoAuditTool if not ready yet
            const openPhotoAudit = () => {
              if (!window.PhotoAuditTool) {
                console.warn('[MobilePanelController] ⏳ PhotoAuditTool not ready, waiting...');

                // Listen for ready event (only once)
                const readyHandler = () => {
                  console.log('[MobilePanelController] ✅ PhotoAuditTool ready, opening...');
                  this.openPhotoAuditWithData();
                };

                document.addEventListener('photoAuditTool:ready', readyHandler, { once: true });

                // Timeout fallback (5 seconds)
                setTimeout(() => {
                  if (!window.PhotoAuditTool) {
                    console.error('[MobilePanelController] ❌ PhotoAuditTool failed to load');
                    alert('Công cụ Photo Audit không tải được. Vui lòng tải lại trang.');
                  }
                }, 5000);
              ...
              // PhotoAuditTool đã ready, mở ngay
              this.openPhotoAuditWithData();
            };
    ...
    actionBtns.forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (action === 'action-close') {
          ...
          // Integrate with existing action logic if needed
        }
      });
    });
  ...
  injectViewFullDetailLink() {
    ...
    // Tìm nơi chèn link
    const detailLower =
      this.detailPanel.querySelector('.detail-lower') ||
  ...
  destroy() {
    ...
    this.boundHandlers.forEach((handlerData, key) => {
      if (handlerData.element && handlerData.handler) {
        ...
        handlerData.element.removeEventListener('scroll', handlerData.handler, {
          passive: true,
        });
