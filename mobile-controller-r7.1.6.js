...
class MobilePanelController {
  ...
  bindResultCardClicks() {
    ...
    /**
   * ✅ NEW R7.1.4: Open PhotoAuditTool with auto-fill data
   */
  openPhotoAuditWithData() {
    if (!window.PhotoAuditTool || typeof window.PhotoAuditTool.openFromMoldDetail !== 'function') {
      console.error('[MobilePanelController] ❌ PhotoAuditTool.openFromMoldDetail not available');
      alert('Công cụ Photo Audit có lỗi, vui lòng liên hệ admin');
      return;
    }
    
    // 4. Get currently selected mold data (if any) for auto-fill
    let moldData = null;
    
    // Try to get from last selected item
    const selectedItemId = document.body.dataset.selectedItemId || document.body.dataset.lastSelectedItemId;
    
    if (selectedItemId && window.DataManager && window.DataManager.data) {
      const allData = window.DataManager.data;
      
      // Search in molds
      let mold = allData.molds.find(m => 
        m.MoldID === selectedItemId || m.MoldCode === selectedItemId
      );
      
      // Search in cutters
      if (!mold && allData.cutters) {
        const cutter = allData.cutters.find(c => 
          c.CutterID === selectedItemId || c.CutterNo === selectedItemId
        );
        
        if (cutter) {
          moldData = {
            moldId: cutter.CutterID,
            moldCode: cutter.CutterNo,
            moldName: cutter.CutterName || cutter.CutterNo
          };
        }
      }
      
      // Convert to PhotoAudit format
      if (mold) {
        moldData = {
          moldId: mold.MoldID,
        ...
        console.log('[MobilePanelController] Auto-fill Photo Audit with selected mold:', moldData);
      }
    }
    
    // 5. Open PhotoAuditTool with auto-fill data
    try {
      if (moldData) {
        // Open with mold data
        window.PhotoAuditTool.openFromMoldDetail(moldData);
        console.log('[MobilePanelController] ✅ Photo Audit opened with auto-fill');
      } else {
        // Open without data (user will input manually)
        window.PhotoAuditTool.openFromSearchPage();
      ...
      item.addEventListener('click', (e) => {
          ...
          switch(tab) {
              ...
              case 'photo-audit':
                ...
                // 4. Get currently selected mold data (if any) để auto-fill
                let moldData = null;
                
                // Try to get from last selected item
                const selectedItemId = document.body.dataset.selectedItemId || 
                                      document.body.dataset.lastSelectedItemId;
                ...
                if (selectedItemId && window.DataManager && window.DataManager.data) {
                    ...
                    // Search in molds
                    let mold = allData.molds.find(m => 
                        m.MoldID === selectedItemId || 
                        m.MoldCode === selectedItemId
                    );
                    
                    // Search in cutters
                    if (!mold && allData.cutters) {
                        const cutter = allData.cutters.find(c => 
                            c.CutterID === selectedItemId || 
                            c.CutterNo === selectedItemId
                        );
                        ...
                        if (cutter) {
                            mold = {
                                ...
                                moldcode: cutter.CutterNo,
                                moldname: cutter.CutterName || cutter.CutterNo,
                                dimensionL: cutter.CavL,
                                dimensionW: cutter.CavW,
                                dimensionD: cutter.CavD,
                                type: 'cutter'
                            };
                        }
                    }
                    ...
                    if (mold) {
                        moldData = {
                            id: mold.MoldID || mold.id,
                            moldcode: mold.MoldCode || mold.moldcode,
                            moldname: mold.MoldName || mold.moldname,
                            dimensionL: mold.dimensionL || mold.CavL || '',
                            dimensionW: mold.dimensionW || mold.CavW || '',
                            dimensionD: mold.dimensionD || mold.CavD || '',
                            type: mold.type || 'mold'
                        };
                        
                        console.log('✅ Auto-fill Photo Audit with selected mold:', moldData);
                    }
                }
                
                // 5. Open PhotoAuditTool settings với auto-fill data
                try {
                    if (moldData) {
                        // Open with mold data
                        window.PhotoAuditTool.openSettings({ mold: moldData });
                        console.log('✅ Photo Audit opened with auto-fill');
                    } else {
                        // Open without data (user will input manually)
                        window.PhotoAuditTool.openSettings();
                        console.log('✅ Photo Audit opened (manual entry mode)');
                    }
                } catch (error) {
                    console.error('[MobilePanelController] ❌ Error opening Photo Audit:', error);
                    alert('写真監査ツールを開く際にエラーが発生しました\n\nLỗi khi mở Photo Audit Tool\n\n' + error.message);
                }
