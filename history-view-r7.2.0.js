/**
 * =============================================================================
 * history-view-r7.2.0.js - Unified History View Module
 * 
 * CHANGELOG r7.2.0 (2025-12-24 16:51 JST):
 * ✅ FIX: Filter group default state changed to collapsed (hidden)
 * ✅ NEW: Badge stats clickable - Filter by Total/Audit/Move/IN-OUT
 * ✅ FIX: Cutter display logic - Use CutterName/CutterCode instead of CutterID
 * 
 * CHANGELOG r7.1.9 (2025-12-19 09:30 JST):
...
(function() {
  ...
  const HistoryView = {
      ...
      dateFilterEnabled: false,
          dateFilterEnabled: false,
    // r7.2.0: IN/OUT filter flag
    inoutFilterActive: false,
    ...
    buildHistoryEvents(locationlog, shiplog, statuslogs, molds, cutters, companies, employees, destinations) {
      ...
      (locationlog || []).forEach(row => {
        ...
        // r7.2.0: Fix cutter display - prioritize CutterName/CutterCode
const itemCode = itemObj 
    ? (hasMold ? itemObj.MoldID : (itemObj.CutterName || itemObj.CutterCode || itemObj.CutterID)) 
    : itemId;
const itemName = itemObj 
    ? (hasMold ? (itemObj.Name || itemObj.MoldName) : (itemObj.CutterName || itemObj.Name))
    : "";
  ...
  const triggers = document.querySelectorAll('[data-history-view-trigger]');
    ...
    bindInsideEvents() {
      ...
      if (clearNotificationsBtn) {
        ...
        // ✅ r7.2.0 NEW - Badge stats clickable to filter
        const statBadges = [
          { id: 'history-stat-total', preset: 'all' },
          { id: 'history-stat-audit', preset: 'audit' },
          { id: 'history-stat-move', preset: 'location' },
          { id: 'history-stat-io', preset: 'inout' }
        ];

        statBadges.forEach(({ id, preset }) => {
          const badge = document.getElementById(id);
          if (badge) {
            // Make it look clickable
            badge.style.cursor = 'pointer';
            badge.parentElement.style.cursor = 'pointer';
            
            badge.parentElement.addEventListener('click', (e) => {
              e.preventDefault();
              e.stopPropagation();
              
              console.log('[HistoryView] Badge clicked:', preset);
              
              // ✅ FIX: Clear filters manually WITHOUT auto-render
    ...
    open(preset) {
      ...
      if (this.els.root) this.els.root.classList.add('hist-open');

      // r7.2.0: Apply collapsed state to DOM on open
      if (this.state.filterGroupCollapsed) {
          if (this.els.filterToggle) this.els.filterToggle.classList.add('collapsed');
          if (this.els.filterContent) this.els.filterContent.classList.add('collapsed');
      }
    },
    ...
    // r7.2.0: Update stats - USE allEvents for FIXED display
    updateStats() {
      // ✅ FIX: Use allEvents for accurate totals, not filtered
      const all = this.state.allEvents;
      const total = all.length;
      const audit = all.filter(ev => ev.ActionKey === ACTION.AUDIT).length;
      const location = all.filter(ev => ev.ActionKey === ACTION.LOCATION_CHANGE).length;
      const inout = all.filter(ev =>
      ...
      console.log('[HistoryView] Stats updated (fixed totals):', { total, audit, location, inout });
    },
