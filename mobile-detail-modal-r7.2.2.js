/* ========================================================================
   MOBILE DETAIL MODAL CONTROLLER R7.1.7
   ========================================================================
...
class MobileDetailModal {
    constructor() {
        ...
        // ✅ Sync initial inventory mode from InventoryCore r8.0
        this.inventoryMode = !!(window.InventoryCore?.getState?.().active);

        // Fallback: legacy InventoryState
        if (this.inventoryMode === false && window.InventoryState) {
    ...
    bindEvents() {
        ...
        // R7.2.1 - Listen for inventory audit completion - ĐÓNG MODAL
        document.addEventListener('inventorydetailModalRequestClose', (e) => {
            console.log('[MobileModal] inventorydetailModalRequestClose received, closing modal');
            this.hide(); // Đóng modal chi tiết
        });

        document.addEventListener('inventorydetailModalClose', (e) => {
            console.log('[MobileModal] inventorydetailModalClose received, closing modal');
            this.hide(); // Đóng modal chi tiết
        });

        // ✅ Listen for inventory session started/ended (InventoryCore r8.0)
        document.addEventListener('inventory:sessionStarted', (e) => {
        console.log('[MobileModal] Inventory session started:', e.detail);
        ...
        document.addEventListener('inventory:sessionEnded', (e) => {
        console.log('[MobileModal] Inventory session ended');
        ...
        // Legacy compatibility: inventoryModeChanged
        document.addEventListener('inventoryModeChanged', (e) => {
        const active = e.detail;
        console.log('[MobileModal] Legacy event inventoryModeChanged:', active);
    ...
    show(item, type = 'mold') {
        ...
        // ✅ Đồng bộ lại chế độ kiểm kê khi mở modal (InventoryCore r8.0)
        const coreState = window.InventoryCore?.getState?.();
        this.inventoryMode = !!(coreState?.active);

        // Fallback: legacy InventoryState
        if (!coreState && window.InventoryState) {
        this.inventoryMode = !!(window.InventoryState?.active);
        }
    ...
    */
    ...
    /* ============================================================================
 * R7.1.7 TRIGGER PHOTO AUDIT – TƯƠNG THÍCH R2.3.0
 * - Ưu tiên dùng PhotoAuditTool.openFromMoldDetail(data)
 * - Hỗ trợ cả mold & cutter, truyền đủ moldId/moldCode/moldName
 * ============================================================================ */

triggerPhotoAudit(item, type) {
  console.log('[MobileModal] triggerPhotoAudit', item, type);

  if (!item) {
    console.error('[MobileModal] No item provided');
    alert('Không có dữ liệu để chụp ảnh / データがありません');
    return;
  }

  // Đóng modal chi tiết trước để nhường màn hình cho Photo Audit
  this.hide();
  ...
  setTimeout(() => {
    if (!window.PhotoAuditTool) {
      ...
      alert('Công cụ Photo Audit chưa sẵn sàng. Vui lòng tải lại trang.\nPhoto Audit Tool がまだ読み込まれていません。');
      return;
    }
    ...
    try {
      ...
      const autoFillData = {
        moldId: isMold ? (item.MoldID || null) : null,
        moldCode: isMold
          ? (item.MoldCode || item.MoldDesignCode || item.MoldDesignCodeJA || '')
          : (item.CutterNo || item.CutterDesignCode || ''),
        moldName: isMold
          ? (item.MoldName || '')
          : (item.CutterName || item.CutterDesignCode || '')
      };

      console.log('[MobileModal] Auto-fill for PhotoAuditTool:', autoFillData);

      // Ưu tiên API mới openFromMoldDetail()
      if (typeof window.PhotoAuditTool.openFromMoldDetail === 'function') {
        window.PhotoAuditTool.openFromMoldDetail(autoFillData);
        console.log('[MobileModal] ✅ Photo Audit opened via openFromMoldDetail');
      } else {
        // Fallback: openSettings() + preselectMold()
        if (typeof window.PhotoAuditTool.openSettings === 'function') {
          window.PhotoAuditTool.openSettings();
        }

        const codeForPreselect =
          autoFillData.moldCode ||
          autoFillData.moldName ||
          autoFillData.moldId ||
          '';

        if (codeForPreselect && typeof window.PhotoAuditTool.preselectMold === 'function') {
          window.PhotoAuditTool.preselectMold(codeForPreselect);
          console.log('[MobileModal] ✅ Photo Audit preselectMold:', codeForPreselect);
        }

        console.log('[MobileModal] Photo Audit opened in fallback mode');
      }
    } catch (error) {
      console.error('[MobileModal] Error opening Photo Audit', error);
      alert('Lỗi khi mở Photo Audit Tool: ' + error.message);
    }
  }, 350); // phù hợp với animation đóng modal
}
    ...
    async handleInventoryAudit() {
    // Lấy item đang mở trong modal
    const item = this.currentItem;
    const itemType = this.currentItemType || 'mold';

    if (!item) {
        alert('Lỗi: Không có dữ liệu vật phẩm kiểm kê');
        return;
    }

    const itemId = itemType === 'cutter' ? item.CutterID || item.id : item.MoldID || item.id;

    if (!itemId) {
        alert('Lỗi: Không tìm thấy ID (MoldID/CutterID) kiểm kê');
        return;
    }
    ...
    // Ghi kiểm kê qua InventoryCore r8.0
    try {
        if (window.InventoryCore && typeof window.InventoryCore.recordAuditSingle === 'function') {
        // ✅ Gọi API mới của InventoryCore r8.0
        const result = await window.InventoryCore.recordAuditSingle(String(itemId), itemType);
        
        if (result.success) {
            console.log('[MobileModal] Audit recorded:', result);
            
            // Re-render UI sau khi audit
            setTimeout(() => {
        ...
        console.warn('[MobileModal] InventoryCore not available');
        alert('Hệ thống kiểm kê chưa sẵn sàng. Vui lòng tải lại trang.');
        }
    } catch (err) {
        console.error('[MobileModal] handleInventoryAudit failed:', err);
        alert('Lỗi khi ghi kiểm kê: ' + (err?.message || err));
    }
    }
