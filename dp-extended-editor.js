...
(function (global) {
  ...
  function normalizeType(v) { var x = low(v); return x === 'cutter' ? 'cutter' : (x === 'tray' ? 'tray' : 'mold'); }
  ...
  function getFieldLabels(k) {
    var m = {
      ...
      CustomerID: ['顧客ID', 'Khách hàng'], CompanyID: ['会社ID', 'Công ty'], KeeperCompany: ['保管会社', 'Công ty lưu kho'],
  ...
  function selectRows(ctx, fieldKey) {
    ...
    if (k === 'CompanyID' || k === 'KeeperCompany' || k === 'CurrentCompanyID') return { kind: 'select', id: 'CompanyID', rows: rows(data, 'companies') };
  ...
  function orderedFields(table, row) {
    ...
    actual.forEach(function (k) {
      ...
      if (/storage_?company/i.test(k)) return; // Completely hide any legacy storage company fields
      out.push(k);
    });
  ...
  function recordKey(dp) {
    ...
    var id = type === 'cutter' ? t(it.CutterID || it.CutterNo || it.code) : (type === 'tray' ? t(it.TrayID) : t(it.MoldID || it.MoldCode || it.code));
    return type + ':' + id;
  }
  ...
  function buildContext(dp) {
    ...
    if (type === 'cutter') {
      ...
      baseRow = byId(rows(data, 'trays'), 'TrayID', item.TrayID);
    } else {
    ...
    var companyId = t((customer && customer.CompanyID) || (baseRow && (baseRow.KeeperCompany || baseRow.CompanyID || baseRow.CurrentCompanyID)) || item.KeeperCompany || item.CompanyID);
    ...
    return {
      ...
      item: item,
      type: type,
      primary: type === 'cutter' ? 'cutters' : (type === 'tray' ? 'trays' : 'molds'),
      baseRow: baseRow,
      design: design,
      customer: customer,
      company: company,
  ...
  function renderValue(ctx, fieldKey, value) {
    ...
    if (/ID$/i.test(fieldKey) || fieldKey === 'KeeperCompany') cls += ' is-id';
